<?php
	...
	*/
	...
	class ezSQL_mysqli extends ezSQLcore
	{
      ...
      /**
       * Impact Plus's database quick access variables
      **/
      $this->attachments  = MAIN_DB_PREFIX."attachments";
  		$this->groups       = MAIN_DB_PREFIX."groups";
      $this->groups_rel   = MAIN_DB_PREFIX."groups_rel";
  		$this->messages     = MAIN_DB_PREFIX."messages";
      $this->online       = MAIN_DB_PREFIX."online";
      $this->online_rel   = MAIN_DB_PREFIX."online_rel";
  		$this->relation     = MAIN_DB_PREFIX."relation";
      $this->settings     = MAIN_DB_PREFIX."settings";
  		$this->users        = USER_TB_NAME;
      $this->nusers       = MAIN_DB_PREFIX."users";
      
      $this->notif        = MAIN_DB_PREFIX."notif";
      $this->logs         = MAIN_DB_PREFIX."logs";
      $this->admin        = MAIN_DB_PREFIX."admin";
		...
		*/
		...
		function select($dbname='', $encoding='')
		{
			...
			if ( ! $dbname )
			{
				...
				$this->show_errors ? throw_error($ezsql_mysqli_str[3]) : null;
			}
			...
			else if ( ! $this->dbh )
			{
				...
				$this->show_errors ? throw_error($ezsql_mysqli_str[4]) : null;
			}
			...
			else if ( !@$this->dbh->select_db($dbname) )
			{
				...
				$this->show_errors ? throw_error($str) : null;
			}
			else
			{
				$this->dbname = $dbname;
